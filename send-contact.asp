...
Dim contact_name, contact_email, contact_subject, contact_message
...
If honeypot <> "" Then
    ...
    isSent = True
ElseIf contact_name <> "" And contact_email <> "" Then
    Dim ObjSendMail
    ...
    ObjSendMail.Subject = "New Contact Message: " & contact_subject
    ...
    emailBody = "<html><head><style>body{font-family:Arial,sans-serif;color:#333;}table{border-collapse:collapse;width:100%;max-width:600px;}td{padding:8px;border:1px solid #ddd;}</style></head><body>" & _
                "<h2>New Contact Message - İncity Rent a Car</h2>" & _
                "<table>" & _
                "<tr><td><strong>Full Name</strong></td><td>" & Server.HTMLEncode(contact_name) & "</td></tr>" & _
                "<tr><td><strong>Email Address</strong></td><td>" & Server.HTMLEncode(contact_email) & "</td></tr>" & _
                "<tr><td><strong>Subject</strong></td><td>" & Server.HTMLEncode(contact_subject) & "</td></tr>" & _
                "<tr><td><strong>Message</strong></td><td>" & Replace(Server.HTMLEncode(contact_message), vbCrLf, "<br>") & "</td></tr>" & _
                "</table></body></html>"
...
<head>
    ...
    <title>Contact Status | İncity Rent a Car Fethiye</title>
...
<header class="header dark-header">
        <!-- Top Bar -->
        <div class="top-bar" style="background-color: #1a1a1a; padding: 10px 0; border-bottom: 1px solid #333; text-align: center;">
            <div class="container">
                <p style="margin: 0; color: #e0e0e0; font-size: 14px; font-weight: 500;">
                    TURSAB NO: 13863 - A GRUBU SEYAHAT ACENTASI - Emiroğlu Tur. Sey. Taş. Ltd. Şti.
                </p>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-lg-8 mx-auto" style="background-color: #1a1a1a; padding: 50px; border-radius: 10px; border: 1px solid #333;">
                <% If isSent Then %>
                    ...
                    <h2 class="mt-4" style="color: #fff; font-weight: 700;">Message Sent Successfully!</h2>
                    <p class="mt-3" style="color: #e0e0e0; font-size: 18px;">Thank you for contacting İncity Rent a Car. We have received your message and will respond to your inquiry via email shortly.</p>
                <% Else %>
                    ...
                    <p class="mt-3" style="color: #e0e0e0; font-size: 18px;">We encountered an issue while sending your message via email: <strong><%= errorMsg %></strong></p>
                    <p style="color: #e0e0e0; font-size: 16px;">Please try sending your request via WhatsApp or contact us directly.</p>
                <% End If %>
                <div class="mt-5">
                    <a href="contact.html" class="btn-style-four" style="background-color: #e5ab1c; border-color: #e5ab1c; color: #111; border-radius: 5px; font-size: 16px; padding: 12px 30px;">Go Back</a>
